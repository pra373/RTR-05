...
GLfloat color[3] = {1.0f,0.85f,0.7f};

//explode factor uniform

GLuint explodeFactorUniform = 0;

GLfloat explodeFactor[3] = { 0.0f, 0.0f, 0.0f };
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szAppName,
		TEXT("Prathamesh Laxmikant Paropkari :    Explosion Effect  "),
...
int initialize(void)
{
	...
	const GLchar* geometryShaderSourceCode =
		...
		"in vec2 oTexCoord[];"
		"\n"
		"out vec3 GStransformedNormals;\n"
		"out vec3 GSlightDirection;\n"
		"out vec3 GSviewerVector;\n"
		"out vec2 GSTexCoord;"
		"uniform vec3 uExplodeFactor;"
		"void main(void)\n"
		"{\n"
		...
		"for(int i = 0; i < 3; i++)\n"
		"{\n"
			"gl_Position = gl_in[i].gl_Position + vec4(uExplodeFactor * normal ,0.0);\n"
			"GStransformedNormals = otransformedNormals[i];\n"
			"GSlightDirection = olightDirection[i];\n"
			"GSviewerVector = oviewerVector[i];\n"
	...
	const GLchar* fragmentShaderSourceCode =
		"#version 460 core\n"
		"\n"
		"in vec3 GStransformedNormals;\n"
		"in vec3 GSlightDirection;\n"
		"in vec3 GSviewerVector;\n"
		"in vec2 GSTexCoord;\n"
		"uniform vec3 ulightAmbient;\n"
		"uniform vec3 ulightDiffuse;\n"
		"uniform vec3 ulightSpecular;\n"
		...
		"if(uKeyPress == 1)\n"
		"{\n"
		"vec3 normalizedTransformedNormals = normalize(GStransformedNormals);\n"
		"vec3 normalizedLightDirection = normalize(GSlightDirection);\n"
		"vec3 normalizedViwerVector = normalize(GSviewerVector);\n"
		...
		"else\n"
		"{\n"
		"PhongADSLight = uColor.rgb; \n"
		"}\n"
		"FragColor =  vec4 (PhongADSLight, 1.0);\n"
		"if(useTexture)"
		"{\n"
		"     FragColor = texture(uTexture,GSTexCoord);\n "
		"}\n"
	...
	explodeFactorUniform = glGetUniformLocation(shaderProgramObject, "uExplodeFactor");
...
void display(void)
{
	...
	translationMatrix = vmath::translate(0.0f, -1.3f, zTranslate);
	...
	glUniform3fv(explodeFactorUniform, 1, explodeFactor);
...
void update(void)
{
	...
	if (bEnableExplode == TRUE)
	{
		explodeFactor[0] += 0.001f;
		explodeFactor[1] += 0.001f;
		explodeFactor[2] += 0.001f;
