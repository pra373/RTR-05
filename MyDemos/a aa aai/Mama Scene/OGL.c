...
GLfloat xTopRightQuad = 1.3f;
GLfloat xBottomLeftQuad = -1.3f;
...
void resize(int width, int height)
{
	...
	gluPerspective(45.0f, ((GLsizei)width / (GLfloat)height), 0.1f, 100.0f);
...
void update(void)
{
	...
	{
		xTopLeftQuad = xTopLeftQuad + 0.0005f;
	}

	if (xBottomRightQuad > -0.1f)
	{
		xBottomRightQuad = xBottomRightQuad - 0.0005f;
	}

	if (xTopRightQuad > -0.1f && xTopLeftQuad >= 0.1)
	{
		xTopRightQuad = xTopRightQuad - 0.0005f;
	}

	if (xBottomLeftQuad < 0.1f && xTopLeftQuad >= 0.1)
	{
		xBottomLeftQuad = xBottomLeftQuad + 0.0005f;
	}
