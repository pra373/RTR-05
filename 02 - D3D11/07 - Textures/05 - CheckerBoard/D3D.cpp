...
uint8_t checkImage[CHECK_IMAGE_HEIGHT][CHECK_IMAGE_WIDTH][4];
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(
		WS_EX_APPWINDOW,
		szAppName,
		TEXT("Prathamesh Laxmikant Paropkari            Direct3D   -  CheckerBoard"),
...
HRESULT initialize(void)
{
	...
	d3dBufferDesc.ByteWidth = sizeof(quadPosition);
	...
	memcpy(d3dMappedSubResource.pData, quadPosition, sizeof(quadPosition));
	...
	d3dBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	d3dBufferDesc.ByteWidth = sizeof(quadTexcoords);
	d3dBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	d3dBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	//create vertex buffer using above structure

	hr = gpID3D11Device->CreateBuffer(&d3dBufferDesc, NULL, &gpID3D11Buffer_TexCoordBuffer);
	...
	gpID3D11DeviceContext->Map(gpID3D11Buffer_TexCoordBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &d3dMappedSubResource);

	//now copy actual data

	memcpy(d3dMappedSubResource.pData, quadTexcoords, sizeof(quadTexcoords));
...
void LoadD3DTexture(void)
{
	...
	//create texture for checker board

	D3D11_TEXTURE2D_DESC d3dTexture2DDesk;

	ZeroMemory((void*)&d3dTexture2DDesk, sizeof(D3D11_TEXTURE2D_DESC));

	d3dTexture2DDesk.Width = (UINT)CHECK_IMAGE_WIDTH;
	d3dTexture2DDesk.Height = (UINT)CHECK_IMAGE_HEIGHT;
	...
	d3dTexture2DDesk.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
	d3dTexture2DDesk.BindFlags = D3D11_BIND_SHADER_RESOURCE;
	d3dTexture2DDesk.CPUAccessFlags = 0;
	d3dTexture2DDesk.MiscFlags = 0;

	ID3D11Texture2D* piID3D11Texture2D_CheckerBoard = NULL;

	D3D11_SUBRESOURCE_DATA d3dSubresourceData;

	ZeroMemory((void*)&d3dSubresourceData, sizeof(D3D11_SUBRESOURCE_DATA));

	d3dSubresourceData.pSysMem = checkImage;
	//d3dSubresourceData.SysMemPitch = (sizeof(checkImage) * 4);

	d3dSubresourceData.SysMemPitch = (CHECK_IMAGE_WIDTH * 4);

	hr = gpID3D11Device->CreateTexture2D(&d3dTexture2DDesk, &d3dSubresourceData, &piID3D11Texture2D_CheckerBoard);
	...
	if (FAILED(hr))
	{
		...
		piID3D11Texture2D_CheckerBoard->Release();

		piID3D11Texture2D_CheckerBoard = NULL;
	...
	ZeroMemory((void*)&srvDesc, sizeof(D3D11_SHADER_RESOURCE_VIEW_DESC));
	srvDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
	srvDesc.ViewDimension = D3D11_SRV_DIMENSION_TEXTURE2D;
	srvDesc.Texture2D.MostDetailedMip = 0;
	srvDesc.Texture2D.MipLevels = -1;

	gpID3D11Device->CreateShaderResourceView(piID3D11Texture2D_CheckerBoard, &srvDesc, &gpID3D11ShaderResourceView);

	if (FAILED(hr))
	{
		gpFile = fopen(gszLogFileName, "a+");
		fprintf(gpFile, "CreateShaderResourceView() for checkerboard failed !!\n\n");
		fclose(gpFile);

		if (piID3D11Texture2D_CheckerBoard) piID3D11Texture2D_CheckerBoard->Release();
		return;
	}
	else
	{
		gpFile = fopen(gszLogFileName, "a+");
		fprintf(gpFile, "CreateShaderResourceView() for checkerboard success !!\n\n");
		fclose(gpFile);
	}
...
/*//create an empty texture according to the new changed size we will call it as depth texture
	...
	gpID3D11Device->CreateShaderResourceView()*/
...
//	d3dSubresourceData.SysMemPitch = (CHECK_IMAGE_WIDTH * 4);
//
//	hr = gpID3D11Device->CreateTexture2D(&d3dTexture2DDesk, NULL &piID3D11Texture2D_CheckerBoard);
//
//	if (FAILED(hr))
//	{
//		gpFile = fopen(gszLogFileName, "a+");
...
//	// now map our position buffer with above resource
//
//	gpID3D11DeviceContext->Map(gpiD3d11Buffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &d3dMappedSubResource);
//
//	//now copy actual data
//
//	memcpy(d3dMappedSubResource.pData, quadPosition, sizeof(quadPosition));
...
//
//
//	/*hr = CreateWICTextureFromMemory(gpID3D11Device, gpID3D11DeviceContext,);*/
//
//}
...
void display(void)
{
	...
	rotationMatrix = XMMatrixRotationY(40.0f);
...
void makeCheckImage(void)
{
	...
	for (i = 0; i < CHECK_IMAGE_HEIGHT; i++)
	{
		for (j = 0; j < CHECK_IMAGE_WIDTH; j++)
		{
			...
			checkImage[i][j][0] = (uint8_t)c;
			checkImage[i][j][1] = (uint8_t)c;
			checkImage[i][j][2] = (uint8_t)c;
			checkImage[i][j][3] = (uint8_t)255;
		}
	}
