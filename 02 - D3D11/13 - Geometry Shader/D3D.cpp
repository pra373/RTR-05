...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(
		WS_EX_APPWINDOW,
		szAppName,
		TEXT("Prathamesh Laxmikant Paropkari            Direct3D   -  Geometry Shader"),
...
HRESULT initialize(void)
{
	...
	const char* geometryShaderSourceCode =
		...
		"};\n"
		"[maxvertexcount(9)]\n"
		"void main(triangle vertexOutput input[3], inout TriangleStream<geometryOutput> triangleStream)\n"
		"{\n"
	...
	hr = D3DCompile(geometryShaderSourceCode, lstrlenA(geometryShaderSourceCode) + 1, "GS", NULL, D3D_COMPILE_STANDARD_FILE_INCLUDE, "main", "gs_5_0", 0, 0, &pID3DBlob_GeometryShaderSourceCode, &pID3DBlob_Error);
	...
	if (FAILED(hr))
	{
		...
		fprintf(gpFile, "Geometry Shader Compiler Error = %s\n\n", (char*)pID3DBlob_Error->GetBufferPointer());
	...
	else
	{
		...
		fprintf(gpFile, "Geometry Shader Compilation success !!\n\n");
	...
	hr = gpID3D11Device->CreateGeometryShader(pID3DBlob_GeometryShaderSourceCode->GetBufferPointer(), pID3DBlob_GeometryShaderSourceCode->GetBufferSize(), NULL, &gpID3D11GeometryShader);
	...
	if (FAILED(hr))
	{
		...
		fprintf(gpFile, "Geometry Shader Creation failed\n\n");
	...
	else
	{
		...
		fprintf(gpFile, "Geometry Shader Creation success !!\n\n");
	...
	const float trianglePosition[] =       // follow clock wise vertex order in Direct3D unlike OpenGL
	{
		0.0f, 1.0f, 0.0f,
		1.0f,-1.0f,0.0f,
		-1.0f,-1.0f,0.0f
	...
	memcpy(d3dMappedSubResource.pData, trianglePosition, sizeof(trianglePosition));
...
void display(void)
{
	...
	translationMatrix = XMMatrixTranslation(0.0f, 0.0f, 2.5f);
