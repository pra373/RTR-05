...
struct CBUFFER
{
	...
	XMVECTOR LightAmbient[2];
	XMVECTOR LightDiffuse[2];
	XMVECTOR LightSpecular[2];
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(
		WS_EX_APPWINDOW,
		szAppName,
		TEXT("Prathamesh Laxmikant Paropkari            Direct3D   -  Two Static Lights On Moving Pyramid (Per Vertex Lighting)"),
...
HRESULT initialize(void)
{
	...
	// Create vertex buffer from above triangle position
	...
	d3dBufferDesc.ByteWidth = sizeof(pyramid_Position);
	...
	if (FAILED(hr))
	{
		...
		fprintf(gpFile, "CreateBuffer() For Position failed!!\n\n");
	...
	else
	{
		...
		fprintf(gpFile, "CreateBuffer() For Position succeded!!\n\n");

		fclose(gpFile);
	}

	// We will set above buffer into pipeline not here but in display() becoz our buffer is DYNAMIC..
	...
	memcpy(d3dMappedSubresource.pData, pyramid_Position, sizeof(pyramid_Position));
	...
	d3dBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	d3dBufferDesc.ByteWidth = sizeof(pyramid_Normals);
	d3dBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	d3dBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	...
	if (FAILED(hr))
	{
		...
		fprintf(gpFile, "CreateBuffer() For Normal failed!!\n\n");
	...
	else
	{
		...
		fprintf(gpFile, "CreateBuffer() For Normal succeded!!\n\n");
	...
	// now map our normal buffer with above resource

	gpID3D11DeviceContext->Map(gpID3D11Buffer_NormalBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &d3dMappedSubresource);

	// Now copy actual data

	memcpy(d3dMappedSubresource.pData, pyramid_Normals, sizeof(pyramid_Normals));
...
void display(void)
{
	...
	rotationMatrixY = XMMatrixRotationY(-XMConvertToRadians(angle_Pyramid));

	worldMatrix = rotationMatrixY * translationMatrix;

	//worldMatrix = XMMatrixTranslation(0.0f, 0.0f, 4.5f);
