...
struct CBUFFER
{
	XMMATRIX WorldViewMatrix;  // model view matrix
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(
		WS_EX_APPWINDOW,
		szAppName,
		TEXT("Prathamesh Laxmikant Paropkari            Direct3D   -  Diffuse Light: Cube"),
...
HRESULT initialize(void)
{
	...
	d3dBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	d3dBufferDesc.ByteWidth = sizeof(cubeNormals);
	d3dBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	d3dBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	...
	memcpy(d3dMappedSubResource.pData, cubeNormals, sizeof(cubeNormals));
...
void display(void)
{
	...
	if (bLightingEnabled == TRUE)
	{
		...
		//XMStoreFloat3(&constantBuffer.Ld, XMVectorSet(1.0f, 1.0f, 1.0f, 1.0f));

		//XMStoreFloat3(&constantBuffer.Kd, XMVectorSet(0.5f, 0.5f, 0.5f, 1.0f));
