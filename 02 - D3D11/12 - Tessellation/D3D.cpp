...
// interface for tessallation Shader
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(
		WS_EX_APPWINDOW,
		szAppName,
		TEXT("Prathamesh Laxmikant Paropkari            Direct3D   -  Tessellation Live Code"),
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:                 // use for non character keys
		...
		case VK_UP:
			...
			if (uiNumberOfLineSegments >= 25)
			{
				uiNumberOfLineSegments = 25;
			}
...
HRESULT initialize(void)
{
	...
	hr = D3DCompile(HullShaderSourceCode, lstrlenA(HullShaderSourceCode) + 1, "HS", NULL, D3D_COMPILE_STANDARD_FILE_INCLUDE, "main", "hs_5_0", 0, 0, &pID3DBlob_HullShaderSourceCode, &pID3DBlob_Error);
	...
	if (FAILED(hr))
	{
		...
		fprintf(gpFile, "Hull Shader Compiler Error = %s\n\n", (char*)pID3DBlob_Error->GetBufferPointer());
	...
	else
	{
		...
		fprintf(gpFile, "Hull Shader Compilation success !!\n\n");
	...
	hr = gpID3D11Device->CreateHullShader(pID3DBlob_HullShaderSourceCode->GetBufferPointer(), pID3DBlob_HullShaderSourceCode->GetBufferSize(), NULL, &gpID3D11HullShader);
	...
	if (FAILED(hr))
	{
		...
		fprintf(gpFile, "Hull Shader Creation failed\n\n");
	...
	else
	{
		...
		fprintf(gpFile, "HULL Shader Creation success !!\n\n");
	...
	hr = D3DCompile(DomainShaderSourceCode, lstrlenA(DomainShaderSourceCode) + 1, "DS", NULL, D3D_COMPILE_STANDARD_FILE_INCLUDE, "main", "ds_5_0", 0, 0, &pID3DBlob_DomainShaderSourceCode, &pID3DBlob_Error);
	...
	if (FAILED(hr))
	{
		...
		fprintf(gpFile, "Domain Shader Compiler Error = %s\n\n", (char*)pID3DBlob_Error->GetBufferPointer());
	...
	else
	{
		...
		fprintf(gpFile, "Domain Shader Compilation success !!\n\n");
	...
	hr = gpID3D11Device->CreateDomainShader(pID3DBlob_DomainShaderSourceCode->GetBufferPointer(), pID3DBlob_DomainShaderSourceCode->GetBufferSize(), NULL, &gpID3D11DomainShader);
	...
	if (FAILED(hr))
	{
		...
		fprintf(gpFile, "Domain Shader Creation failed\n\n");
	...
	else
	{
		...
		fprintf(gpFile, "Domain Shader Creation success !!\n\n");
	...
	const float pointPosition[] =       // follow clock wise vertex order in Direct3D unlike OpenGL
	{
		 -1.0,-1.0,  //third quadrant
		 -0.5,1.0,   //second quadrant
		 0.5,-1.0,  //fourth quadrant
	...
	d3dBufferDesc.ByteWidth = sizeof(pointPosition);
	...
	memcpy(d3dMappedSubResource.pData, pointPosition, sizeof(pointPosition));
	...
	hr = gpID3D11Device->CreateBuffer(&d3dBufferDesc, NULL, &gpID3D11Buffer_ConstantBuffer_HullShader);
	...
	if (FAILED(hr))
	{
		...
		fprintf(gpFile, "gpID3D11Device -> CreateBuffer() for HULL shader constant buffer failed\n\n");
	...
	else
	{
		...
		fprintf(gpFile, "gpID3D11Device -> CreateBuffer() for Hull Shader constant buffer success !!\n\n");
	...
	hr = gpID3D11Device->CreateBuffer(&d3dBufferDesc, NULL, &gpID3D11Buffer_ConstantBuffer_DomainShader);
	...
	if (FAILED(hr))
	{
		...
		fprintf(gpFile, "gpID3D11Device -> CreateBuffer() for Domain Shader constant buffer failed\n\n");
	...
	else
	{
		...
		fprintf(gpFile, "gpID3D11Device -> CreateBuffer() for Domain Shader constant buffer success !!\n\n");
	...
	hr = gpID3D11Device->CreateBuffer(&d3dBufferDesc, NULL, &gpID3D11Buffer_ConstantBuffer_PixelShader);
	...
	if (FAILED(hr))
	{
		...
		fprintf(gpFile, "gpID3D11Device -> CreateBuffer() for pixel shader constant buffer failed\n\n");
	...
	else
	{
		...
		fprintf(gpFile, "gpID3D11Device -> CreateBuffer() for pixel shader constant buffer success !!\n\n");
...
void display(void)
{
	...
	translationMatrix = XMMatrixTranslation(0.0f, 0.0f, 3.5f);
	...
	//update line strips and line segments in constant buffer of Hull shader

	CBUFFER_HULL_SHADER constantBuffer_HullShader;

	ZeroMemory((void*)&constantBuffer_HullShader, sizeof(CBUFFER_HULL_SHADER));

	constantBuffer_HullShader.Hull_Constant_Function_Params = XMVectorSet(1.0f, uiNumberOfLineSegments, 0.0f, 0.0f);

	gpID3D11DeviceContext->UpdateSubresource(gpID3D11Buffer_ConstantBuffer_HullShader, 0, NULL, &constantBuffer_HullShader, 0, 0); 

	TCHAR str[255];

	wsprintf(str, TEXT("Prathamesh Laxmikant Paropkari        Direct3D - Tesselation Program     [Number Of Line Segments = %d] "), uiNumberOfLineSegments);

	SetWindowText(ghwnd, str);

	//************************************************************************************************************************

	//update Line Color in constant buffer of pixel shader
