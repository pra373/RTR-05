...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(
		WS_EX_APPWINDOW,
		szAppName,
		TEXT("Prathamesh Laxmikant Paropkari            Direct3D   -  3D Rotation Two Shapes Colored"),
...
// position
	...
	d3dBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	d3dBufferDesc.ByteWidth = sizeof(pyramidColors);
	d3dBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	d3dBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	//create vertex buffer using above structure

	hr = gpID3D11Device->CreateBuffer(&d3dBufferDesc, NULL, &gpID3D11Buffer_ColorBuffer_Pyramid);
	...
	gpID3D11DeviceContext->Map(gpID3D11Buffer_ColorBuffer_Pyramid, 0, D3D11_MAP_WRITE_DISCARD, 0, &d3dMappedSubResource);

	//now copy actual data

	memcpy(d3dMappedSubResource.pData, pyramidColors, sizeof(pyramidColors));

	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_ColorBuffer_Pyramid, 0);

	// ********************************************   Position buffer for cube   ***********************************************
	...
	if (FAILED(hr))
	{
		...
		fprintf(gpFile, "gpID3D11Device -> CreateBuffer() for position buffer for cube failed\n\n");
	...
	else
	{
		...
		fprintf(gpFile, "gpID3D11Device -> CreateBuffer() for position buffer for cube success !!\n\n");
	...
	d3dBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	d3dBufferDesc.ByteWidth = sizeof(cubeColors);
	d3dBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	d3dBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	//create vertex buffer using above structure

	hr = gpID3D11Device->CreateBuffer(&d3dBufferDesc, NULL, &gpID3D11Buffer_ColorBuffer_Cube);
	...
	if (FAILED(hr))
	{
		...
		fprintf(gpFile, "gpID3D11Device -> CreateBuffer() for Color buffer for Cube failed\n\n");
	...
	else
	{
		...
		fprintf(gpFile, "gpID3D11Device -> CreateBuffer() for Color buffer for Cube success !!\n\n");
	...
	gpID3D11DeviceContext->Map(gpID3D11Buffer_ColorBuffer_Cube, 0, D3D11_MAP_WRITE_DISCARD, 0, &d3dMappedSubResource);

	//now copy actual data

	memcpy(d3dMappedSubResource.pData, cubeColors, sizeof(cubeColors));

	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_ColorBuffer_Cube, 0);
...
void display(void)
{
	...
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_ColorBuffer_Pyramid, &stride, &offset);
	...
	finalrotationMatrix = XMMatrixIdentity();
	...
	scaleMatrix = XMMatrixScaling(0.73f, 0.73f, 0.73f);

	finalrotationMatrix = rotationMatrixX * rotationMatrixY * rotationMatrixZ;



	worldMatrix = scaleMatrix * finalrotationMatrix * translationMatrix;
	...
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_ColorBuffer_Cube, &stride, &offset);
