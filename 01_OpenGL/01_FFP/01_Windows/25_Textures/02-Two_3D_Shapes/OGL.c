...
int initialize(void)
{
	...
	if (bResult == FALSE)
	{
		fprintf(gpFile, "Loading of stone texture failed !! Exiting now\n");
		return(-6);
	}
	...
	if (bResult == FALSE)
	{
		fprintf(gpFile, "Loading of Kundali texture failed !! Exiting now\n");
		return(-7);
	}

	//Tell OpenGL to enable the Texture

	glEnable(GL_TEXTURE_2D);     // step 4 in notebook
...
void display(void)
{
	...
	glRotatef(C_Angle, 0.0f, 1.0f, 0.0f);              // when you use seperate gl_rotate() call for each axis it will show a different effect
	...
	glBindTexture(GL_TEXTURE_2D, 0);  //step 15
