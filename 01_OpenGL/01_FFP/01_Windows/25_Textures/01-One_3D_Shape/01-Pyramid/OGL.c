...
int initialize(void)
{
	...
	bResult = loadGLTexture(&TEXTURE_UCHIHA, MAKEINTRESOURCE(UCHIHACREST));

	if (bResult == FALSE)
	{
		fprintf(gpFile, "Loading of uchiha texture failed !! Exiting now\n");
		return(-6);
	}
...
void display(void)
{
	...
	glBindTexture(GL_TEXTURE_2D, TEXTURE_UCHIHA);
...
void uninitialize(void)
{
   ...
   // if application is exiting in fullScreen then ha pudcha code follow kela jael
	...
	if (TEXTURE_UCHIHA)
	{
		glDeleteTextures(1, &TEXTURE_UCHIHA);
		TEXTURE_UCHIHA = 0;
	}
