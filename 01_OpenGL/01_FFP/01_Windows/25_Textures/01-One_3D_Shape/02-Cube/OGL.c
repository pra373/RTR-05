...
int initialize(void)
{
	...
	bResult = loadGLTexture(&TEXTURE_UZUMAKI, MAKEINTRESOURCE(UZUMAKICREST));

	if (bResult == FALSE)
	{
		fprintf(gpFile, "Loading of Uzumaki texture failed !! Exiting now\n");
		return(-6);
	}
...
void display(void)
{
	...
	glBindTexture(GL_TEXTURE_2D, TEXTURE_UZUMAKI);
	glBegin(GL_QUADS);
