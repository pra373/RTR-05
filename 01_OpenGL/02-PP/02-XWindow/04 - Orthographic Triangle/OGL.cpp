...
mat4 orthographicProjectionMatrix;  //using 4*4 matrix.
...
int main(void)
{
    ...
    XStoreName(display, window, "Prathamesh Paropkari - Orthographic Triangle");
...
int initialize(void)
{
	...
	glCompileShader(fragmentShaderObject);
	 ...
	 const GLfloat trianglePosition[] = { 0.0f, 100.0f, 0.0f,
										 -100.0f,-100.0f,0.0f,
										  100.0f,-100.0f,0.0f
										};
...
void resize(int width ,int height)
{
	...
	if (width <= height)
	{
		/*glOrtho(-100.0f,
			...
			100.0f * ((GLfloat)height / (GLfloat)width),
			-100.0f,
			100.0f);*/     //dont work in PP

		//use orthographic projection function from vmath.h
