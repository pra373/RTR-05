...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer, OnDoubleTapListener, OnGestureListener
{
	...
	private void display()
	{
		...
		pushMatrix(modelMatrix);
		{
			...
			Matrix.multiplyMM(modelViewProjectionMatrix, 0, perspectiveProjectionMatrix, 0, tempMatrix, 0);

			GLES32.glUniformMatrix4fv(mvpMatrixUniform, 1, false, modelViewProjectionMatrix,0);

			GLES32.glUniform4f(colorUniform, 1.0f, 0.5f, 0.0f, 1.0f);
			...
			float[] eyRotationMatrix = new float[16];
			float[] edRotationMatrix = new float[16];
			float[] translationMatrix1 = new float[16];
			float[] scaleMatrix1 = new float[16];
			...
			Matrix.setIdentityM(eyRotationMatrix,0);
			Matrix.setIdentityM(edRotationMatrix,0);
			Matrix.setIdentityM(translationMatrix1,0);
			Matrix.setIdentityM(scaleMatrix1,0);
			...
			Matrix.translateM(translationMatrix1,0, 1.1f, 0.0f, 0.0f);
			Matrix.setRotateM(eyRotationMatrix,0,(float)year, 0.0f, 1.0f, 0.0f);
			Matrix.setRotateM(edRotationMatrix,0,(float)day, 0.0f, 1.0f, 0.0f);


			// Perform matrix multiplications
			Matrix.multiplyMM(tempMatrix1, 0, modelMatrix, 0, eyRotationMatrix, 0);
			Matrix.multiplyMM(tempMatrix2, 0, tempMatrix1, 0, translationMatrix1, 0);
			Matrix.multiplyMM(tempMatrix3, 0, tempMatrix2, 0, edRotationMatrix, 0);
			System.arraycopy(tempMatrix3, 0, modelMatrix, 0, 16); 

			pushMatrix(modelMatrix);
			{
				Matrix.scaleM(scaleMatrix1,0,0.6f,0.6f,0.6f);
				Matrix.multiplyMM(modelMatrix,0,modelMatrix,0,scaleMatrix1,0);

				Matrix.setIdentityM(tempMatrix1,0);

				Matrix.multiplyMM(tempMatrix1,0,perspectiveProjectionMatrix,0,viewMatrix,0);
				Matrix.multiplyMM(modelViewProjectionMatrix,0,tempMatrix1,0,modelMatrix,0);

				GLES32.glUniformMatrix4fv(mvpMatrixUniform, 1, false, modelViewProjectionMatrix,0);

				GLES32.glUniform4f(colorUniform, 0.4f, 0.9f, 1.0f, 1.0f);
			...
			float[] translateMatrix3 = new float[16];
			float[] myRotationMatrix = new float[16];
			float[] mdRotationMatrix = new float[16];
			float[] scaleMatrix2 = new float[16];
			
		

			Matrix.setIdentityM(translateMatrix3,0);
			Matrix.setIdentityM(myRotationMatrix,0);
			Matrix.setIdentityM(mdRotationMatrix,0);
			Matrix.setIdentityM(scaleMatrix2,0);
			...
			Matrix.translateM(translateMatrix3,0,1.0f,0.0f,0.0f);

			Matrix.setRotateM(myRotationMatrix,0,moon_year,0.0f,1.0f,0.0f);
			Matrix.setRotateM(mdRotationMatrix,0,moon_day,0.0f,1.0f,0.0f);


			// Perform matrix multiplications
			Matrix.multiplyMM(tempMatrix1, 0, modelMatrix, 0, myRotationMatrix, 0);
			Matrix.multiplyMM(tempMatrix2, 0, tempMatrix1, 0, translateMatrix3, 0);
			Matrix.multiplyMM(tempMatrix3, 0, tempMatrix2, 0, mdRotationMatrix, 0);
			System.arraycopy(tempMatrix3, 0, modelMatrix, 0, 16);
			
			pushMatrix(modelMatrix);
			{
				Matrix.scaleM(scaleMatrix2,0,0.3f,0.3f,0.3f);
				Matrix.multiplyMM(modelMatrix,0,modelMatrix,0,scaleMatrix2,0);
	...
	private void update()
	{
		...
		if(singleTap == 1)
		{
			year += 1.0f; // Adjust the speed as necessary
			if (year >= 360.0f) 
			{
				year -= 360.0f;
			}
