...
//import android.graphics.BitmapFactory;  //no need of this package in case of procedural texture
...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer, OnDoubleTapListener, OnGestureListener
{
	...
	byte[] checkImage = new byte[checkImageWidth * checkImageHeight * 4];
	...
	private int[] checkerBoardTexture = new int[1];
	...
	private void initialize(GL10 gl)
	{
		...
		GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, rectangleTexCoord.length * 4, texCoordBuffer, GLES32.GL_STATIC_DRAW);
		...
		GLES32.glClearColor(0.7f,0.7f,0.7f,1.0f);

		//load texture

		checkerBoardTexture[0] = loadGLTexture();
	...
	//variable declaration
		...
		float[] square1Position = new float[18];
		float[] square2Position = new float[18];
		//code
		...
		square1Position[0] = 0.0f;  //right top
		...
		square1Position[6] = -2.0f;  //left bottom1
		square1Position[7] = -1.0f;
		square1Position[8] = 0.0f;

		square1Position[9] = -2.0f;  //left bottom2
		square1Position[10] = -1.0f;
		square1Position[11] = 0.0f;

		square1Position[12] = 0.0f; //right bottom
		square1Position[13] = -1.0f;
		square1Position[14] = 0.0f;

		square1Position[15] = 0.0f;  //right top
		...
		square2Position[0] = 2.41421f;   //right top
		...
		square2Position[6] = 1.0f;   //left bottom1
		square2Position[7] = -1.0f;
		square2Position[8] = 0.0f;

		square2Position[9] = 1.0f;   //left bottom2
		...
		square2Position[15] = 2.41421f;   //right top
		...
		GLES32.glBindTexture(GLES32.GL_TEXTURE_2D, checkerBoardTexture[0]);
	...
	//step 1 : allocate enough size buffer for triangle vertices
		...
		ByteBuffer byteBuffer_PT1 = ByteBuffer.allocateDirect(square1Position.length * 4);

		//step 2: set byte order

		byteBuffer_PT1.order(ByteOrder.nativeOrder());

		//step3 : convert created buffer as float array

		FloatBuffer positionBuffer_PT1 = byteBuffer_PT1.asFloatBuffer();

		//step 4 : fill buffer with vertices array

		positionBuffer_PT1.put(square1Position);

		//step 5 : rewind the buffer to 0th index

		positionBuffer_PT1.position(0);

		GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, vbo[0]);
		GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, square1Position.length * 4, positionBuffer_PT1, GLES32.GL_DYNAMIC_DRAW);
		...
		ByteBuffer byteBuffer_PT2 = ByteBuffer.allocateDirect(square2Position.length * 4);

		//step 2: set byte order

		byteBuffer_PT2.order(ByteOrder.nativeOrder());

		//step3 : convert created buffer as float array

		FloatBuffer positionBuffer_PT2 = byteBuffer_PT2.asFloatBuffer();

		//step 4 : fill buffer with vertices array

		positionBuffer_PT2.put(square2Position);

		//step 5 : rewind the buffer to 0th index

		positionBuffer_PT2.position(0);

		GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, vbo[0]);
		GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, square2Position.length * 4, positionBuffer_PT2, GLES32.GL_DYNAMIC_DRAW);
	...
	private void uninitialize()
	{
		...
		if(checkerBoardTexture[0] > 0)
		{
			GLES32.glDeleteTextures(1, checkerBoardTexture,0);
			checkerBoardTexture[0] = 0;
		}
	...
	private int loadGLTexture()
	{
		...
		//create ByteBuffer from our array

		ByteBuffer byteBuffer_ch = ByteBuffer.allocateDirect(checkImageWidth * checkImageHeight * 4);

		byteBuffer_ch.order(ByteOrder.nativeOrder());

		byteBuffer_ch.put(checkImage);

		byteBuffer_ch.position(0);

		//create the bitmap from array given by above function  
		...
		bitmap.copyPixelsFromBuffer(byteBuffer_ch);
