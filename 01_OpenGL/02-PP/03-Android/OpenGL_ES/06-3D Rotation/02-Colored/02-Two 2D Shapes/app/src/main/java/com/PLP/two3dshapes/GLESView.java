...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer, OnDoubleTapListener, OnGestureListener
{
	...
	private void initialize(GL10 gl)
	{
		...
		ByteBuffer byteBufferColored = ByteBuffer.allocateDirect(pyramidColor.length * 4);

		//step 2: set byte order

		byteBufferColored.order(ByteOrder.nativeOrder());

		//step3 : convert created buffer as float array

		FloatBuffer colorBufferPyramid = byteBufferColored.asFloatBuffer();
		...
		ByteBuffer byteBuffer_R = ByteBuffer.allocateDirect(cubePosition.length * 4);
		...
		positionBuffer_r.put(cubePosition);

		//step 5 : rewind the buffer to 0th index

		positionBuffer_r.position(0);

		GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, cubePosition.length * 4, positionBuffer_r, GLES32.GL_STATIC_DRAW);
		...
		ByteBuffer byteBuffer_C = ByteBuffer.allocateDirect(cubeColor.length * 4);
		...
		FloatBuffer colorBufferCube = byteBuffer_C.asFloatBuffer();
	...
	private void display()
	{
		...
		float[] rotationMatrix = new float[16]; // New rotation matrix
		...
		Matrix.translateM(modelViewMatrix,0, 1.8f, 0.0f, -8.0f);
