...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) 
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, 
							szAppName,
							TEXT(" Prathamesh Laxmikant Paropkari : \t  Tessellated Triangle"),
...
int initialize(void)
{
	...
	static const GLchar* VertexShaderSourceCode =  "#version 460 core\n"
												   "                 \n"
												   "void main(void)  \n"
												   "{                \n"
												   "  vec4 vertex1 = vec4(-0.5, -0.5, 0.0f, 1.0f); \n"  //position of vertex 1
		                                           "  vec4 vertex2 = vec4(0.5, -0.5, 0.0f, 1.0f); \n"   //position of vertex 2
		                                           "  vec4 vertex3 = vec4(0.0, 0.5, 0.0f, 1.0f); \n"    //position of vertex 3
		                                           "\n"
												   " if ( gl_VertexID == 0 ) "
												   "{                                       \n"
                                                   "  gl_Position = vertex1;                \n"
	...
	static const GLchar* controlShaderSourceCode =
		"#version 460 core \n"
	...
	static const GLchar* evaluationShaderSourceCode =
		"#version 460 core\n"
		"                 \n"
	...
	static const GLchar* fragmentShaderSourceCode = 
		"#version 460 core \n"
		"                 \n"
		"out vec4 color;  \n"
		"                 \n"
	...
	if (status == GL_FALSE)
	{
		...
		if (infoLogLength > 0)
		{
			...
			{
				// step 5 (d) : get the compilation error log info into memory allocated string
				glGetShaderInfoLog(fragmentShader, infoLogLength, NULL, szInfoLog);
	...
	if (controlShader == 0)
	{
		fprintf(gpFile, "glCreateShader() failed !! for Tessellation control shader");
		return(-3);
	}

	glShaderSource(controlShader, 1, (const GLchar**)&controlShaderSourceCode, NULL);
	...
	//step 5 : Check for vertex shader compilation
	//step 5 (a) :  get compile status in status variable
	glGetShaderiv(controlShader, GL_COMPILE_STATUS, &status);

	//step 5 (b) : if error occur i.e status == GL_FALSE,  get length of error message into infoLogLength variable
	if (status == GL_FALSE)
	{

		glGetShaderiv(controlShader, GL_INFO_LOG_LENGTH, &infoLogLength);
		...
		if (infoLogLength > 0)
		{
			...
			{
				// step 5 (d) : get the compilation error log info into memory allocated string
				glGetShaderInfoLog(controlShader, infoLogLength, NULL, szInfoLog);
	...
	//creates empty shader object to copy the source code of evaluation shader

	evaluationShader = glCreateShader(GL_TESS_EVALUATION_SHADER);

	if (evaluationShader == 0)
	{
		fprintf(gpFile, "glCreateShader() failed !! for evaluation shader");
		return(-1);
	}

	//copies the shader source code into empty shader object

	glShaderSource(evaluationShader, 1, (const GLchar**)&evaluationShaderSourceCode, NULL);
	...
	//step 5 : Check for vertex shader compilation
	//step 5 (a) :  get compile status in status variable
	glGetShaderiv(evaluationShader, GL_COMPILE_STATUS, &status);

	//step 5 (b) : if error occur i.e status == GL_FALSE,  get length of error message into infoLogLength variable
	if (status == GL_FALSE)
	{

		glGetShaderiv(evaluationShader, GL_INFO_LOG_LENGTH, &infoLogLength);
		...
		if (infoLogLength > 0)
		{
			...
			{
				// step 5 (d) : get the compilation error log info into memory allocated string
				glGetShaderInfoLog(evaluationShader, infoLogLength, NULL, szInfoLog);
	...
	glAttachShader(programObject, controlShader);
	glAttachShader(programObject, evaluationShader);
