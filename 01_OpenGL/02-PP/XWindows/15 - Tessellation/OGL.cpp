...
int main(void)
{
    ...
    XStoreName(display, window, "Prathamesh Paropkari - Tessellation Program");
...
int initialize(void)
{
   ...
   const GLchar* vertexShaderSourceCode =
	...
	if (status == GL_FALSE)
	{
		...
		if (infoLogLength > 0)
		{
			...
			if (szInfoLog != NULL)
			{
				...
				fprintf(gpFile, "Tesselation Control Shader compilation error log : %s\n", szInfoLog);
				free(szInfoLog);
				szInfoLog = NULL;
	...
	glCompileShader(fragmentShaderObject);
	 ...
	 LineColorUniform = glGetUniformLocation(shaderProgramObject, "uLineColor"); //must be done after linkage of shader program
...
void draw(void)
{
    ...
    if(uiNumberOfLineSegments < 25 && uiNumberOfLineSegments > 1)
    {
   ...
   sprintf(str,"Prathamesh Paropkari        Tessellation Program         Line Segments: %d",uiNumberOfLineSegments);
