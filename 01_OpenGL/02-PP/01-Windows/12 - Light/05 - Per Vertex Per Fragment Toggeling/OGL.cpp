...
struct perFragLightSources fLights[2];
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) 
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, 
							szAppName,
							TEXT("Prathamesh Laxmikant Paropkari : Per Vertex Per Fragment Toggling"),
...
// *************************************** Per fragment shader program object starts **********************
	...
	fLights[0].diffuse = vec3(0.0, 1.0, 0.0);
	fLights[1].diffuse = vec3(0.0, 0.0, 1.0);
	fLights[0].specular = vec3(0.0, 1.0, 0.0);
	fLights[1].specular = vec3(0.0, 0.0, 1.0);
	fLights[0].position = vec4(-2.0, 0.0, 0.0, 1.0);
	fLights[1].position = vec4(2.0, 0.0, 1.0, 1.0);
...
void display(void)
{
	...
	/*if (bLightingEnable == TRUE && enablePerVertexLighting == TRUE)
	{
		glUniform1i(keyPressUniform, 1);
	...
	else if (bLightingEnable == TRUE && enablePerFragmentLighting == TRUE)
	{
		glUniform1i(keyPressUniform, 1);
